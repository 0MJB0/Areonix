...
import 'edit_form_dietician_mixin.dart';

mixin EditFormDieticianUIMixin<T extends EditFormDieticianView>
    on ConsumerState<T>, EditFormDieticianMixin<T> {
  CommonAppbar appBar(BuildContext context) {
    return const CommonAppbar(
      title: StringConstants.editFormDieticianTitle,
    );
  }
  ...
  Widget buildFormNameField() {
    return SizedBox(
      width: kIsWeb
          ? Responsiveness.editFormNameWeb.w
          : Responsiveness.editFormNameMobile.w,
  ...
  Widget buildQuestionCard(int questionIndex) {
    return SizedBox(
      width: kIsWeb
          ? Responsiveness.editFormQuestionWeb
          : Responsiveness.editFormQuestionMobile,
      child: Card(
        color: TColor.grey,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              RoundTextFormField(
                controller: questionControllers[questionIndex],
                labelText: StringConstants.createFormDieticianQuestion,
                icon: Icons.edit,
                iconColor: TColor.airforce,
                onChanged: (value) {
                  setState(() {
                    questions[questionIndex] = value; // Soru metnini güncelle
                  });
                },
              ),
              Align(
  ...
  Widget buildSendFormButton() {
    return Container(
      ...
      child: ElevatedButton(
        ...
        onPressed: sendForm,
        child: kIsWeb
            ? const WebText(text: StringConstants.editFormDieticianEditForm)
            : BoldText(
                text: StringConstants.editFormDieticianEditForm,
                color: TColor.black,
              ),
      ),
    );
    ...
    return Expanded(
      child: SingleChildScrollView(
        child: Center(
          child: SizedBox(
            width: kIsWeb
                ? Responsiveness.editFormCardWeb.w
                : Responsiveness.editFormCardMobile.w,
            child: Column(
              children: [
                ListView.builder(
                  shrinkWrap: true,
